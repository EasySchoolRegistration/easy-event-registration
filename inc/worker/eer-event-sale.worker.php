			...
			if (isset($eer_data['valid'])) {
				...
				foreach ($eer_data['valid'] as $ticket_id => $ticket) {
					...
					if (EER()->ticket->eer_is_solo($ticket_id, $ticket_data)) {
						$new_sold_tickets = $this->worker_registration_solo->process_registration($order_id, $ticket_id, $ticket_data, $ticket->number_of_tickets, $ticket, $data->event_id);
					} else {
						$new_sold_tickets = $this->worker_registration_couple->process_registration($order_id, $user_id, $ticket_id, $ticket_data, $ticket, $data->event_id);
					}
