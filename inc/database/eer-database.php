...
class EER_Database
{
	...
	public static function create_tables()
	{
		...
		$sql .= "CREATE TABLE {$wpdb->prefix}eer_events_orders (
			...
			PRIMARY KEY id (id),
			CONSTRAINT {$wpdb->prefix}eer_events_orders_unique_key_u UNIQUE (id, unique_key)
		) $charset_collate;";
		...
		$sql .= "CREATE TABLE {$wpdb->prefix}eer_sold_tickets (
			...
			PRIMARY KEY id (id),
			FOREIGN KEY (ticket_id) REFERENCES {$wpdb->prefix}eer_tickets(id) ON DELETE CASCADE,
			FOREIGN KEY (order_id) REFERENCES {$wpdb->prefix}eer_events_orders(id) ON DELETE CASCADE
		) $charset_collate;";
