...
class EER_Payment {
	...
	public function eer_get_not_payed_payments_by_event($event_id) {
		...
		$results = $wpdb->get_results($wpdb->prepare("SELECT eo.id AS order_id, ep.id AS payment_id, u.ID, u.display_name, u.user_email 
FROM {$wpdb->prefix}eer_events_payments AS ep 
JOIN {$wpdb->prefix}eer_events_orders eo ON ep.order_id = eo.id 
JOIN {$wpdb->users} u ON u.ID = eo.user_id 
WHERE eo.event_id = %d
  AND ep.status = %d", [$event_id, EER_Enum_Payment::NOT_PAID]));
